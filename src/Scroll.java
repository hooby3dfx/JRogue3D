...
class Scroll extends Toy  implements Serializable {
	...
	void readby(){
		switch(kind){
			case Id.LIGHT:
				Room r= level.room_at(owner.row, owner.col);
				if(r!=null && 0==(level.map[owner.row][owner.col]&TUNNEL)){
					owner.tell("the room is lit by a shimmering blue light");
					if(0 != (level.map[owner.row][owner.col]&DARK)){
						View v= owner instanceof Man? ((Man)owner).view : null;
						r.light_room(v);
					}
				}else
					owner.tell("the corridor glows and then fades");
				break;
			case Id.BLANK_PAPER:
				owner.tell("this scroll seems to be blank");
				break;
	...
	void aggravate(Persona man){
		...
		while(e.hasMoreElements()){
			...
			level.self.mark(monster.row, monster.col);
		}
	}
	void hold_monster(){
		int mcount= 0;
