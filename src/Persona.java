...
class Persona extends Item implements Serializable {
	...
	int r_rings= 0;
	int e_rings= 0;
	int str_current= 0;		// Current strength
	int str_max= 0;			// Max strength
	int	add_strength= 0;
	int gold= 0;
	int exp= 0;				// Experience level
	boolean being_held= false;

	int m_flags= 0;			/* monster flags */
...
//			((Man)this).view.msg.message(s, b);
	...
	void remove_curse(){
		if(armor!=null)armor.is_cursed= false;
		if(weapon!=null)weapon.is_cursed= false;
		if(left_ring!=null)left_ring.is_cursed= false;
		if(right_ring!=null)right_ring.is_cursed= false;
	}
}
