...
class Toy extends Item implements Serializable {
	...
	private Toy check_duplicate(Vector pack){
	...
	boolean packcanhold(ItemVector pack){
		if(kind==Id.GOLD)
			return true;
		return null != check_duplicate(pack) || '?' != pack.next_avail_ichar();
	}		
	int get_armor_class(){
	...
	int value(){
		...
		if(0!=(kind&Id.ARMOR))
			v= 100*klass;
		return v + Id.get_value(kind, identified);
	}
	String name(){
