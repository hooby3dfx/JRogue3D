...
public class Room implements Header, Serializable {
	...
	int is_room;
	private int left_col, right_col, top_row, bottom_row;
	...
	static final int R_TREE=   0100;
	static final int R_ROOMISH= R_ROOM|R_MAZE|R_TREE;
	...
	public String toString(){
		...
		if(0 != (is_room & R_TREE))		s= s.concat(" Forest");
	...
	Rowcol nextrc(Rowcol rc){
		if(rc==null)
			return new Rowcol(top_row, left_col);
		return nextrc(rc.row, rc.col);
	}
	Rowcol randomrc(int indent){
		// indent 1 to avoid the walls
		int r= level.self.rand.get(top_row+indent, bottom_row-indent);
		int	c= level.self.rand.get(left_col+indent, right_col-indent);
		return new Rowcol(r,c);
	}
	...
	void mazeroom(){
		if(is_room != R_NOTHING || bottom_row>=level.nrow-2)
			return;
		is_room= R_MAZE;
		if(left_col<2)
			left_col= 2;
		Rowcol rc= randomrc(1);
		make_maze(rc.row, rc.col);
		hide_boxed_passage(top_row, left_col, bottom_row, right_col, level.self.rand.get(2));
	}
	void clearedge(int r, int c){
		if(0!=(level.map[r][c]&DOOR))
			return;
		try{
			if(0!=((level.map[r][c]|level.map[r][c-1]|level.map[r][c+1]
			|level.map[r+1][c]|level.map[r-1][c])&BLOCK))
				return;
	...
	void drop_rox(int n){
		...
		if(0 == (is_room & R_ROOM))
			return;
		is_room &= ~R_ROOM; is_room |= R_TREE;
		int a= (bottom_row - top_row)*(right_col - left_col);
		if(n>a/2)
			n= a/2;
		for(r= top_row; r<=bottom_row; r++)
		for(c= left_col; c<=right_col; c++){
			mask= level.map[r][c] & ~(HORWALL|VERTWALL|DARK);
			level.map[r][c]= mask|FLOOR;
		}
		while(n>0 && nmiss<9){
			Rowcol rc= randomrc(0);
			++nmiss;
			for(int k= 0; k<=8; k+= 2){
				mask= level.map[rc.row+Id.xtab[k]][rc.col+Id.ytab[k]];
				if(0!=(mask&(BLOCK|DOOR)))
					break;
				if(k==8){
					mask &= ~(HOLDER|FLOOR);
					mask |= BLOCK;
					level.map[rc.row][rc.col]= mask;
					--n;
					nmiss= 0;
				}
			}
		...
		// now erode the edges
		n= right_col - left_col + 1;
		int ix[]= level.self.rand.permute(n);
		for(int k= n/3; k<n; k++)
			clearedge(top_row, left_col + ix[k]);
		ix= level.self.rand.permute(n);
		for(int k= n/3; k<n; k++)
			clearedge(bottom_row, left_col + ix[k]);

		n= bottom_row - top_row + 1;
		ix= level.self.rand.permute(n);
		for(int k= n/3; k<n; k++)
			clearedge(top_row+ix[k], left_col);
		ix= level.self.rand.permute(n);
		for(int k= n/3; k<n; k++)
			clearedge(top_row+ix[k], right_col);
	}	
	...
	void visit_rooms(){
		Room r= this;
///System.out.println("Visit "+r);
		r.is_room |= R_VISIT;
		for(int i= 0; i<4; i++)if(doors[i]!=null){
///System.out.println(" door "+i+" of " + r + " " + doors[i]);
	...
	void fill_it(boolean do_rec_de){
		...
		for(i= 0; i < 4; i++){
			Room r= nabes[i];
			if(r==null || 0==(r.is_room & R_ROOMISH))
				continue;
			if(level.same_row(this,r))
				tunnel_dir= left_col < r.left_col? Id.RIGHT : Id.LEFT;
			else if(level.same_col(this, r))
...
*/
	...
	int party_toys(){
		...
		for(int i= 0; i<n; i++){
			for(int j= 0; j<250; j++){
				Rowcol rc= randomrc(1);
				if(0!=(level.map[rc.row][rc.col]&HOLDER)){
					Toy obj= level.gr_toy();
					obj.place_at(rc.row, rc.col, TOY);
					nf++;
					break;
				}
			}
		...
		if(is_maze || level.self.rand.percent(gold_pct)){
			for(int j= 0; j < 50; j++){
				...
				if(0!=(level.map[rc.row][rc.col]&HOLDER)){
					...
					level.plant_gold(rc.row, rc.col, gold);
					break;
				}
			}
		}
